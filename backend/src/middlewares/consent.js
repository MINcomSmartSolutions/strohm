...
const requireConsent = async (req, res, next) => {
    // Use session ID for tracing across requests in the same session
    // Note: sessionID is automatically generated by express-session
    ...
    try {
        ...
        if (user) {
            ...
            if (!sessionUser || sessionUser.user_id !== user.user_id) {
                ...
                log.debug(`Updated session for user ${user.user_id}`);
            }
            const hasConsent = await hasLatestConsent(sessionUser.user_id);
            if (!hasConsent) {
                log.info(`User ${sessionUser.user_id} does not have latest consent - redirecting to consent page`);
                return res.redirect('/consent');
            }
        } else {
            // User doesn't exist in DB yet - they need to give consent first before we create them
            log.info(`New user (oauth_id: ${oidcUser.sub}) - redirecting to consent page`);
            return res.redirect('/consent');
        }
        return next();
    } catch (error) {
